...
sockets.on('listen', function (io) {
	...
	io.sockets.on('connection', function (socket) {
		...
		socket.on('state', safesocket(3, function (id, time, playing) {
			// TODO: Split out validation/loading functions
			if (!Video.validId.test(id)) { callback(null); return; }
			var video = new Video(id);
			metadata.load(video, function (err) {
				// TODO: Some way to abort this callback if another one (which was started later) comes in and finishes first
				// TODO: Caching of video metadata results... and pooling simultaneous pending requests
				if (err) { callback(null); return; }
				runner.state.video = video;
				runner.state.time = time; // TODO: Safer number
				runner.state.playing = playing;
			});
			// TODO: Callback?
		}));
